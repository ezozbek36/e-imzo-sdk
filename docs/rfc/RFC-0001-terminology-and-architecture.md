...
| [NORMALIZED]    | PKCS#7 base64 payload           | `pkcs7`, `pkcs7b64`, `pkcs7_64`          | Cross-plane   | Reviewed local docs/examples and server docs/examples use different literal field names for base64 PKCS#7 material. This RFC uses editorial `PKCS#7 base64 payload` for the shared concept while preserving observed wire names in concrete examples. |
